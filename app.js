...
function getHolidays(year) {
  ...
  // Substitute holidays (振替休日): 祝日が日曜日のとき、その後最も近い「祝日でない日」を休日とする
  ...
  // 国民の休日 (祝日法第3条第3項): 前日と翌日が「国民の祝日」である平日も休日
  // 例: 2026年9月22日(火) は 9/21(敬老の日)と 9/23(秋分の日)に挟まれて休日になる
  const isNamedHoliday = (k) => h[k] && h[k] !== '振替休日';
  for (let m = 0; m < 12; m++) {
    const dim = daysInMonth(year, m);
    for (let d = 1; d <= dim; d++) {
      const key = `${year}-${pad2(m + 1)}-${pad2(d)}`;
      if (h[key]) continue;
      const prevD = new Date(year, m, d - 1);
      const nextD = new Date(year, m, d + 1);
      const pk = `${prevD.getFullYear()}-${pad2(prevD.getMonth() + 1)}-${pad2(prevD.getDate())}`;
      const nk = `${nextD.getFullYear()}-${pad2(nextD.getMonth() + 1)}-${pad2(nextD.getDate())}`;
      if (isNamedHoliday(pk) && isNamedHoliday(nk)) {
        h[key] = '国民の休日';
      }
    }
  }
    ...
    <Fragment>
      ...
      {view === 'staff' && currentUser && (() => {
        ...
        return <StaffView user={live} year={year} month={month} setYear={setYear} setMonth={setMonth}
          ...
          monthAssignments={monthAssignments} isConfirmed={isConfirmed} assignments={assignments} staffById={staffById}
...
function Header({ user, onLogout, subtitle, onSwitchToStaff, adminBacklink }) {
  ...
  return (
    <header className="bg-emerald-950 text-amber-50">
      <div className="max-w-7xl mx-auto px-3 md:px-6 py-3 md:py-4 flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2 md:gap-3 min-w-0">
          <div className="display text-lg md:text-2xl whitespace-nowrap">休日当番 <span className="italic opacity-70">Roster</span></div>
        ...
        <div className="flex items-center gap-1.5 md:gap-3 flex-wrap justify-end">
          {onSwitchToStaff && (
            <button onClick={onSwitchToStaff} className="jp-sans text-[10px] md:text-xs px-2 md:px-3 py-1 md:py-1.5 border border-amber-50/30 hover:bg-emerald-900">スタッフ画面</button>
          )}
          {adminBacklink && (
            <button onClick={adminBacklink} className="jp-sans text-[10px] md:text-xs px-2 md:px-3 py-1 md:py-1.5 border border-amber-50/30 hover:bg-emerald-900">管理者画面</button>
          )}
          {user && (
            <div className="flex items-center gap-1.5 md:gap-2">
              <div className="w-6 h-6 md:w-7 md:h-7 flex items-center justify-center jp-serif text-xs text-white" style={{ backgroundColor: c.hex }}>{user.name.slice(0, 1)}</div>
              <div className="text-right">
                <div className="jp-serif text-xs md:text-sm flex items-center gap-1 md:gap-1.5">
                  <span className="hidden sm:inline">{user.name}</span>
                  <span className="sm:hidden">{user.name.split(' ')[1] || user.name}</span>
                  {user.role === 'admin' && <span className="jp-sans text-[8px] md:text-[9px] px-1 md:px-1.5 py-0.5 bg-amber-50/10 border border-amber-50/30">管理者</span>}
                </div>
              </div>
            </div>
          )}
          <button onClick={onLogout} className="jp-sans text-[10px] md:text-xs px-2 md:px-3 py-1 md:py-1.5 hover:bg-emerald-900">ログアウト</button>
        </div>
      </div>
    </header>
  );
...
/* ───────── Staff View ───────── */
function StaffView({ user, year, month, setYear, setMonth, submissions, saveSubmission, monthAssignments, isConfirmed, assignments, staffById, onLogout, push, adminBacklink }) {
  ...
  const cycle = (d) => {
    ...
    // 次の状態を計算（setXは非同期なので、保存用に明示的に新しい配列を作る）
    let nextPref = preferred, nextAv = available, nextUn = unavailable;
    const isPref = preferred.includes(d), isAv = available.includes(d), isUn = unavailable.includes(d);
    if (!isPref && !isAv && !isUn)      nextAv   = [...available, d];
    else if (isAv)                       { nextAv = available.filter(x => x !== d); nextPref = [...preferred, d]; }
    else if (isPref)                     { nextPref = preferred.filter(x => x !== d); nextUn = [...unavailable, d]; }
    else                                 nextUn   = unavailable.filter(x => x !== d);
    setPreferred(nextPref); setAvailable(nextAv); setUnavailable(nextUn);
    setSubmitted(false);
    // 自動下書き保存（月をまたいでも消えないように）
    saveSubmission(user.id, { preferred: nextPref, available: nextAv, unavailable: nextUn, submitted: false });
  };
          ...
          <div className="lg:col-span-2 bg-white border border-stone-200 p-6">
            ...
            <div className="grid grid-cols-7 gap-1.5">
              ...
              {Array.from({ length: days }, (_, i) => i + 1).map(d => {
                const assigneeId = monthAssignments[d];
                const assignee = assigneeId ? staffById[assigneeId] : null;
                return (
                  <PreferenceCell key={d} day={d} dow={new Date(year, month, d).getDay()}
                    ...
                    assignee={assignee} isMine={assigneeId === user.id} />
                );
              })}
            </div>
            ...
            {!isConfirmed && (
              <div className="mt-6 pt-5 border-t border-stone-200 flex flex-wrap items-center gap-3 justify-between">
                <div className="jp-sans text-xs text-stone-600 leading-relaxed">
                  クリックで「未入力 → ○可能 → ◎是非やりたい → ×不可能」と切替<br/>
                  <span className="text-emerald-700">✓ 入力は自動で下書き保存されます</span>。月を切り替えても消えません。最後に「提出する」を押してください。<br/>
                  <span className="text-stone-500">担当が確定済みの日は色付きで担当者名が表示されます。★があなたの当番日です。</span>
                </div>
...
function PreferenceCell({ day, dow, duty, holiday, pref, avail, una, locked, onCycle, assignee, isMine }) {
  if (!duty) {
  ...
  // 担当者が決まっている場合: 担当者の色で塗りつぶし表示（管理者画面の DutyCell に近い見た目）
  if (assignee) {
    const ac = colorOf(assignee);
    const cellStyle = isMine
      ? { backgroundColor: ac.hex, borderColor: ac.hex, color: '#fff' }
      : { backgroundColor: ac.bg,  borderColor: ac.hex, borderLeftWidth: '5px', color: ac.text };
    const dayTextColor = isMine ? '#fff' : (dow === 0 ? '#b91c1c' : dow === 6 ? '#1d4ed8' : ac.text);
    return (
      <button disabled={locked} onClick={onCycle}
        className={`relative aspect-square flex flex-col p-1 lg:p-1.5 jp-serif border-2 transition-all select-none ${locked ? 'cursor-default' : 'cursor-pointer'}`}
        style={cellStyle}>
        <div className="flex items-start justify-between w-full leading-none">
          <span className="text-sm font-medium" style={{ color: dayTextColor }}>{day}</span>
          <div className="flex items-center gap-0.5">
            {isMine && <span className="text-[10px] leading-none">★</span>}
            {holiday && <span className={`text-[8px] px-1 leading-tight ${isMine ? 'bg-white/25 text-white' : 'bg-white/60 text-red-700'}`}>祝</span>}
          </div>
        </div>
        {holiday && <span className="text-[8px] leading-tight truncate mt-0.5" style={{ color: dayTextColor, opacity: 0.85 }}>{holiday}</span>}
        <div className="mt-auto flex items-center gap-1 min-w-0">
          {!isMine && <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: ac.hex }} />}
          <span className="text-[10px] lg:text-xs font-medium truncate" style={{ color: dayTextColor }}>
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-3 md:px-6 py-4 md:py-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4 md:mb-6">
          <div>
            <p className="jp-sans text-[10px] md:text-xs tracking-widest text-stone-500 mb-1">ADMIN DASHBOARD</p>
            <h2 className="display text-2xl md:text-3xl text-emerald-950">当番管理ダッシュボード</h2>
          </div>
          <div className="flex gap-1 border border-stone-300 bg-white self-start md:self-auto">
            {[{id:'month',label:'月間管理'},{id:'annual',label:'年度集計'},{id:'staff',label:'スタッフ管理'}].map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`jp-sans text-xs md:text-sm px-3 md:px-4 py-1.5 md:py-2 ${tab===t.id ? 'bg-emerald-900 text-amber-50' : 'text-stone-600 hover:bg-stone-50'}`}>{t.label}</button>
            ))}
          </div>
        </div>
        {tab === 'month'  && <MonthTab {...props} />}
        {tab === 'annual' && <AnnualTab {...props} />}
...
function MonthTab({ year, month, setYear, setMonth, staff, staffById, monthAssignments, submissionStatus, submittedCount, isConfirmed, onAutoAssign, onReassign, onClearDay, onConfirm, onUnconfirm, submissions, push, assignments }) {
  ...
  return (
    <Fragment>
      <div className="flex flex-col md:flex-row md:flex-wrap items-stretch md:items-center justify-between gap-3 mb-3 bg-white border border-stone-200 px-3 md:px-5 py-3">
        <MonthNav year={year} month={month} setYear={setYear} setMonth={setMonth} />
        <div className="flex flex-wrap items-center gap-2 justify-end">
          {isConfirmed ? (
            <Fragment>
              ...
              <button onClick={doConfirmAndSync} disabled={syncing} className="jp-sans text-xs md:text-sm px-3 md:px-4 py-2 bg-blue-700 text-amber-50 hover:bg-blue-800 disabled:opacity-50">
            ...
            <Fragment>
              <button onClick={onAutoAssign} disabled={submittedCount === 0}
                className="jp-sans text-xs md:text-sm px-3 md:px-4 py-2 bg-emerald-900 text-amber-50 hover:bg-emerald-800 disabled:opacity-40">✨ 自動振分</button>
              <button onClick={doConfirmAndSync} disabled={assignedCount === 0 || syncing}
                className="jp-sans text-xs md:text-sm px-3 md:px-4 py-2 bg-amber-700 text-amber-50 hover:bg-amber-600 disabled:opacity-40">
                {syncing ? `同期中 ${syncProg.done}/${syncProg.total}` : '📤 確定 & 転送'}
              </button>
            </Fragment>
          )}
        </div>
      ...
      <div className="flex flex-wrap items-center gap-x-4 md:gap-x-6 gap-y-2 mb-4 bg-stone-50 border border-stone-200 px-3 md:px-5 py-2.5 text-xs jp-sans">
        <span className="flex items-baseline gap-1.5"><span className="text-stone-500">提出</span><span className="display text-base md:text-lg text-stone-800">{submittedCount}/{staff.length}</span><span className="text-stone-400 text-[11px]">{pct}%</span></span>
        <span className="h-4 w-px bg-stone-300" />
        <span className="flex items-baseline gap-1.5"><span className="text-stone-500">割当</span><span className="display text-base md:text-lg text-stone-800">{assignedCount}/{dutyD.length}</span></span>
        <span className="h-4 w-px bg-stone-300" />
        <span className="flex items-baseline gap-1.5"><span className="text-stone-500">未提出</span><span className={`display text-base md:text-lg ${staff.length-submittedCount > 0 ? 'text-amber-700' : 'text-emerald-800'}`}>{staff.length - submittedCount}</span></span>
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-start">
        <div className="bg-white border border-stone-200 p-2 md:p-5 flex-1 min-w-0">
          <div className="grid grid-cols-7 gap-1 lg:gap-1.5 mb-2">
            {dowJP.map((d, i) => (
              <div key={d} className={`text-center jp-serif text-xs lg:text-sm py-1.5 lg:py-2 font-medium ${i === 0 ? 'text-red-700' : i === 6 ? 'text-blue-700' : 'text-stone-500'}`}>{d}</div>
            ))}
          </div>
          <div className="grid grid-cols-7 gap-1 lg:gap-1.5">
            {Array.from({ length: firstDow }).map((_, i) => <div key={'b' + i} className="aspect-square lg:aspect-[5/4]" />)}
            {Array.from({ length: days }, (_, i) => i + 1).map(d => (
...
function DutyCell({ day, dow, duty, holiday, staff, confirmed, onClick }) {
  if (!duty) {
    ...
    return <div className={`aspect-square lg:aspect-[5/4] p-1 lg:p-1.5 ${dowClr}`}><div className="jp-serif text-xs lg:text-sm">{day}</div></div>;
  }
  const dowClr = dow === 0 ? 'text-red-700' : dow === 6 ? 'text-blue-700' : 'text-stone-700';
  ...
  return (
    <button onClick={onClick} disabled={confirmed}
      className={`relative aspect-square lg:aspect-[5/4] p-1.5 lg:p-2 text-left transition-all ${cellCls} ${!confirmed ? 'hover:shadow-md cursor-pointer' : 'cursor-default'}`} style={cellStyle}>
      <div className="flex items-start justify-between">
        <span className={`jp-serif font-medium text-sm lg:text-base ${dowClr}`}>{day}</span>
        <div className="flex items-center gap-1">
          {holiday && <span className="jp-sans text-[8px] lg:text-[9px] text-red-700 bg-red-50 px-1 py-0.5 leading-none">祝</span>}
          {confirmed && staff && <span style={{ color: c.text }} className="text-[9px]">🔒</span>}
        </div>
      </div>
      {holiday && <div className="jp-sans text-[8px] lg:text-[9px] text-red-600 mt-0.5 leading-tight truncate" title={holiday}>{holiday}</div>}
      <div className="absolute bottom-1 lg:bottom-1.5 left-1 lg:left-2 right-1 lg:right-2">
        {staff ? (
          <div className="flex items-center gap-1 min-w-0">
            <span className="w-2 h-2 lg:w-2.5 lg:h-2.5 rounded-full shrink-0" style={{ backgroundColor: c.hex }} />
            <span className="jp-sans text-[10px] lg:text-xs font-medium truncate" style={{ color: c.text }}>{staff.name.split(' ')[1] || staff.name}</span>
          </div>
        ) : <div className="jp-sans text-[9px] lg:text-[10px] text-red-600 font-medium">未割当</div>}
      </div>
    </button>
  );
}
...
function SubmissionPanel({ staff, submissionStatus, submissions, year, month, monthCounts, submittedCount }) {
  ...
  return (
    <div className="w-full lg:w-[260px] shrink-0 bg-white border border-stone-200 flex flex-col lg:sticky lg:top-3 lg:max-h-[calc(100vh-180px)]">
...
/* ───────── Annual Tab (年度集計: 4月〜翌3月) ───────── */
function AnnualTab({ staff, staffById, assignments, year, month }) {
  // 現在表示している月から、所属する年度を初期値として算出
  const initialFy = month >= 3 ? year : year - 1;
  const [fy, setFy] = useState(initialFy);

  // 年度内の (year, monthIndex 0-11) を順に並べる: 4月,5月,...,12月,1月,2月,3月
  const fyMonths = useMemo(() => {
    const arr = [];
    for (let m = 3; m <= 11; m++) arr.push({ y: fy, m });     // 4-12月
    for (let m = 0; m <= 2;  m++) arr.push({ y: fy + 1, m }); // 翌1-3月
    return arr;
  }, [fy]);
  ...
  const data = useMemo(() => {
    const result = staff.map(s => {
      const monthly = fyMonths.map(({ y, m }) => {
        const days = assignments[monthKey(y, m)] || {};
        return Object.values(days).filter(sid => sid === s.id).length;
      });
      const total = monthly.reduce((a, b) => a + b, 0);
      return { staff: s, monthly, total };
    });
    return result;
  }, [staff, assignments, fyMonths]);

  // 年度全体の当番日数（理論値）と割当済み合計
  const totalDutyDays   = fyMonths.reduce((acc, { y, m }) => acc + dutyDaysOf(y, m).length, 0);
  const totalAssigned   = data.reduce((acc, d) => acc + d.total, 0);
  const expectedPer     = staff.length ? (totalAssigned / staff.length) : 0;
  const maxCount        = Math.max(1, ...data.map(d => d.total));
  const minCount        = Math.min(...data.map(d => d.total));
  const fairnessSpread  = maxCount - minCount;
  ...
  return (
    <Fragment>
      {/* 年度ナビゲーション & サマリー */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-3 bg-white border border-stone-200 px-3 md:px-5 py-3">
        <div className="flex items-center gap-3">
          <button onClick={() => setFy(fy - 1)} className="p-1.5 hover:bg-stone-100 text-stone-600 text-xl">‹</button>
          <div className="text-center min-w-[120px]">
            <div className="display text-2xl md:text-3xl text-emerald-950 leading-none">{fy}<span className="text-base opacity-60 ml-1">年度</span></div>
            <div className="jp-sans text-[10px] md:text-xs text-stone-500 mt-1">{fy}/4 〜 {fy+1}/3</div>
          </div>
          <button onClick={() => setFy(fy + 1)} className="p-1.5 hover:bg-stone-100 text-stone-600 text-xl">›</button>
          <button onClick={() => setFy(initialFy)} className="jp-sans text-[10px] md:text-xs px-2 py-1 border border-stone-300 text-stone-600 hover:bg-stone-50 ml-2">今年度</button>
        </div>
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs jp-sans">
          <span className="flex items-baseline gap-1.5"><span className="text-stone-500">年度の当番日</span><span className="display text-base md:text-lg text-stone-800">{totalDutyDays}</span><span className="text-stone-400">日</span></span>
          <span className="h-4 w-px bg-stone-300 self-center" />
          <span className="flex items-baseline gap-1.5"><span className="text-stone-500">割当済</span><span className="display text-base md:text-lg text-emerald-800">{totalAssigned}</span></span>
          <span className="h-4 w-px bg-stone-300 self-center" />
          <span className="flex items-baseline gap-1.5"><span className="text-stone-500">平均/人</span><span className="display text-base md:text-lg text-stone-800">{expectedPer.toFixed(1)}</span></span>
          <span className="h-4 w-px bg-stone-300 self-center" />
          <span className="flex items-baseline gap-1.5">
            <span className="text-stone-500">差</span>
            <span className={`display text-base md:text-lg ${fairnessSpread <= 1 ? 'text-emerald-800' : fairnessSpread <= 3 ? 'text-amber-700' : 'text-red-700'}`}>{fairnessSpread}</span>
            <span className="text-stone-400 text-[10px]">最多−最少</span>
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
        {[...data].sort((a, b) => b.total - a.total).map(({ staff: s, total }) => {
          const c = colorOf(s);
          const diff = total - expectedPer;
          const pct = expectedPer > 0 ? Math.round((total / expectedPer) * 100) : 0;
          return (
            <div key={s.id} className="bg-white border border-stone-200 px-4 py-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="jp-serif text-sm md:text-base text-stone-900 truncate">{s.name}</div>
                <div className="jp-sans text-[10px] text-stone-500">{c.name}</div>
              </div>
              <div className="text-right">
                <div className="flex items-baseline gap-1">
                  <span className="display text-2xl md:text-3xl" style={{ color: c.text }}>{total}</span>
                  <span className="jp-sans text-xs text-stone-500">回</span>
                </div>
                <div className={`jp-sans text-[10px] ${Math.abs(diff) < 0.5 ? 'text-emerald-700' : diff > 0 ? 'text-amber-700' : 'text-blue-700'}`}>
      ...
      <div className="bg-white border border-stone-200 overflow-x-auto">
        <div className="px-3 md:px-5 py-3 border-b border-stone-200 bg-stone-50">
          <h3 className="jp-serif text-sm md:text-base text-stone-800">月別 担当回数</h3>
          <p className="jp-sans text-[10px] md:text-xs text-stone-500 mt-0.5">数字が大きいほど色が濃く表示されます</p>
        </div>
        <table className="w-full min-w-[640px] text-sm">
          <thead>
            <tr className="border-b border-stone-200 bg-stone-50/50">
              <th className="jp-serif text-xs text-stone-600 text-left px-3 py-2 sticky left-0 bg-stone-50/95 z-10">メンバー</th>
              {monthLabels.map(label => (
                <th key={label} className="jp-sans text-[10px] text-stone-600 px-1 py-2 text-center w-10">{label}</th>
              ))}
              <th className="jp-sans text-xs text-stone-700 px-2 py-2 text-center bg-stone-100">合計</th>
            </tr>
          </thead>
          <tbody>
            {[...data].sort((a, b) => b.total - a.total).map(({ staff: s, monthly, total }) => {
              const c = colorOf(s);
              return (
                <tr key={s.id} className="border-b border-stone-100">
                  <td className="px-3 py-2 sticky left-0 bg-white z-10">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="jp-serif text-xs md:text-sm text-stone-900 truncate">{s.name}</span>
                    </div>
                  </td>
                  {monthly.map((cnt, i) => {
                    const intensity = cnt / maxCount;
                    const bg = cnt === 0 ? 'transparent' : c.bg;
                    const opacity = cnt === 0 ? 1 : 0.5 + intensity * 0.5;
                    return (
                      <td key={i} className="px-1 py-2 text-center">
                        <div className="display text-sm md:text-base inline-flex items-center justify-center w-8 h-8 md:w-9 md:h-9"
                             style={{ backgroundColor: bg, color: cnt === 0 ? '#d6d3d1' : c.text, opacity }}>
                          {cnt || '–'}
                        </div>
                      </td>
                    );
                  })}
                  <td className="px-2 py-2 text-center bg-stone-50">
                    <span className="display text-lg md:text-xl" style={{ color: c.text }}>{total}</span>
                  </td>
                </tr>
              );
            })}
            <tr className="border-t-2 border-stone-300 bg-stone-50/70">
              <td className="px-3 py-2 jp-sans text-xs text-stone-600 sticky left-0 bg-stone-50/95 z-10">月合計</td>
              {fyMonths.map(({ y, m }, i) => {
                const days = assignments[monthKey(y, m)] || {};
                const cnt = Object.keys(days).length;
                const duty = dutyDaysOf(y, m).length;
                return (
                  <td key={i} className="px-1 py-2 text-center">
                    <div className="jp-sans text-[10px] md:text-xs text-stone-700">{cnt}<span className="text-stone-400">/{duty}</span></div>
                  </td>
                );
              })}
              <td className="px-2 py-2 text-center bg-stone-100">
                <span className="display text-lg md:text-xl text-stone-800">{totalAssigned}</span>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <p className="jp-sans text-[10px] md:text-xs text-stone-500 mt-3">
        ※ 年度は4月〜翌3月。確定前の自動振分案も含めて集計しています。<br/>
        　「差」は最多と最少の差です。1以下なら公平な振分、3以上は調整を検討してください。
      </p>
    </Fragment>
  );
}
